...
func resolve_level1_effects():
	...
	var region_selection = await Main.select_land(token_selection["region"].adjacent_regions, false)
	if region_selection == null:
		return
	await Main.push_token(token_selection["token"], token_selection["region"], region_selection, false)
	# Second...
	Main.get_node("LabelContainer").set_text("You may push up to 1 more Explorers from this land.")
	token_selection = await Main.select_invaders_for_removal([token_selection["region"]], true, false, false, true)
	if token_selection == null || token_selection["skipped"]:
		return
	Main.get_node("LabelContainer").set_text("Select a destination.")
	region_selection = await Main.select_land(token_selection["region"].adjacent_regions, false)
	if region_selection == null:
		return
	await Main.push_token(token_selection["token"], token_selection["region"], region_selection, false)
...
func resolve_level2_effects():
	...
	token_selection = await Main.select_invaders_for_removal([token_selection["region"]], true, true, false, true)
	if token_selection == null || token_selection["skipped"]:
		return
	Main.get_node("LabelContainer").set_text("Select a destination.")
	region_selection = await Main.select_land(token_selection["region"].adjacent_regions, false)
